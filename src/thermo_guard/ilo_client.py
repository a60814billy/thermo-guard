...
try:
    ...
    logging.getLogger("thermo-guard").error(
        "Failed to import python-ilorest-library. Make sure it's installed."
    )
    import sys
...
class IloClient:
    ...
    def disconnect(self) -> None:
        ...
        if self.redfish_client:
            try:
                ...
                logger.error(
                    f"Error disconnecting from iLO interface at {self.host}: {e}"
                )
            self.redfish_client = None
    ...
    def power_on(self) -> bool:
        ...
        try:
            ...
            reset_uri = system_response.dict["Actions"]["#ComputerSystem.Reset"][
                "target"
            ]
            reset_response = self.redfish_client.post(
                reset_uri, body={"ResetType": "On"}
            )
            ...
            if reset_response.status == 200:
                ...
                logger.error(
                    f"Failed to power on server at {self.host}: {reset_response.text}"
                )
                return False
